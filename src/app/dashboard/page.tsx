    ...
    return (
        <div className="space-y-10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
                <div className="lg:col-span-2 glass rounded-2xl md:rounded-[2.5rem] p-6 md:p-10">
                    <div className="flex items-center justify-between mb-8">
                        <h3 className="text-xl md:text-2xl font-black text-slate-800">Recent Jobs</h3>
                        <button className="text-blue-600 font-bold hover:underline text-sm md:text-base">View All</button>
                    </div>

                    <div className="space-y-4">
                        {jobs.length === 0 ? (
                            <div className="text-center py-20 text-slate-400 font-bold border-2 border-dashed border-slate-100 rounded-2xl md:rounded-3xl">
                                No jobs found. Create your first job!
                            </div>
                        ) : (
                            jobs.map((job) => (
                                <div key={job.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 md:p-6 bg-slate-50 rounded-xl md:rounded-[1.5rem] border border-slate-100 hover:border-blue-200 transition-all gap-4">
                                    <div className="flex items-center gap-4">
                                        <div className="w-10 h-10 md:w-12 md:h-12 bg-white rounded-lg md:rounded-xl flex items-center justify-center shadow-sm shrink-0">
                                            <HiClipboardCheck className="text-blue-600 text-lg md:text-xl" />
                                        </div>
                                        <div className="min-w-0">
                                            <h4 className="font-black text-slate-800 truncate text-sm md:text-base mb-0.5">{job.job_title} #{job.job_number}</h4>
                                            <p className="text-slate-500 text-[10px] md:text-sm font-medium truncate italic">{job.service_address || 'No address'}</p>
                                        </div>
                                    </div>
                                    <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-center">
                                        <span className={`px-4 py-1 rounded-full text-[9px] font-black uppercase tracking-wider ${job.status === 'completed' ? 'bg-green-100 text-green-700' :
                                        ...
                                        <p className="text-slate-400 text-[9px] md:text-xs font-bold sm:mt-2">
                ...
                <div className="glass rounded-2xl md:rounded-[2.5rem] p-6 md:p-10">
                    <h3 className="text-xl md:text-2xl font-black text-slate-800 mb-8">Low Stock Alerts</h3>
                    <div className="space-y-4">
                        <div className="p-8 text-center bg-slate-50 border border-slate-100 rounded-2xl">
                            <p className="text-slate-400 font-bold text-xs md:text-sm">All items are in stock!</p>
                        </div>
                    </div>
                </div>
            </div>
