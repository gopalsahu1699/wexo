...
import { takePhoto, isNative } from "@/lib/services/mobile";

export default function MemberTaskDetailsClient({ params }: { params: Promise<{ id: string }> }) {
    ...
    if (id === 'current' && !task) {
        return (
            <div className="min-h-screen bg-white flex flex-col items-center justify-center p-10 text-center">
                 <h2 className="text-xl font-black text-slate-900 tracking-widest">NO ACTIVE GIG</h2>
                 <p className="text-slate-400 mt-2 font-bold uppercase text-[10px]">Select a session from the map to begin</p>
                 <button 
                    onClick={() => router.push('/dashboard/member')}
                    className="mt-8 px-8 py-4 bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest"
                >
                    Go Back
                </button>
            </div>
                                ...
                                {task?.status === 'in_progress' && (
                                    <div className="space-y-4">
                                        <div 
                                            ...
                                            className={`bg-white p-12 rounded-[3rem] border-2 border-dashed flex flex-col items-center justify-center gap-3 active:bg-slate-50 transition-colors cursor-pointer group ${capturedPhoto ? 'border-emerald-500 bg-emerald-50/50' : 'border-slate-200'}`}
                                        >
                                            {capturedPhoto ? (
                                                <>
                                                    <div className="w-20 h-20 rounded-2xl overflow-hidden shadow-xl border-4 border-white mb-2">
                                                        <img src={`data:image/jpeg;base64,${capturedPhoto}`} alt="Proof" className="w-full h-full object-cover" />
                                                    </div>
                                                    <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Snapshot Captured</span>
                                                </>
                                            ) : (
                                                <>
